#include <avr/io.h>
#include <stdio.h>
#include <stdlib.h>

unsigned char S[] = {
	 };

unsigned char mul2(unsigned char a) {
	unsigned char result = a << 1u;
	if (a & 0x80u) {
		result ^= 0x1Bu;
	}
	return result;
}

unsigned char* mixColumns(unsigned char state[16]) {
	static unsigned char result[16];
	result[0] = mul2(state[0]) ^ mul2(state[4]) ^ state[4] ^ state[8] ^ state[12];
	result[1] = mul2(state[1]) ^ mul2(state[5]) ^ state[5] ^ state[9] ^ state[13];
	result[2] = mul2(state[2]) ^ mul2(state[6]) ^ state[6] ^ state[10] ^ state[14];
	result[3] = mul2(state[3]) ^ mul2(state[7]) ^ state[7] ^ state[11] ^ state[15];
	result[4] = state[0] ^ mul2(state[4]) ^ mul2(state[8]) ^ state[8] ^ state[12];
	result[5] = state[1] ^ mul2(state[5]) ^ mul2(state[9]) ^ state[9] ^ state[13];
	result[6] = state[2] ^ mul2(state[6]) ^ mul2(state[10]) ^ state[10] ^ state[14];
	result[7] = state[3] ^ mul2(state[7]) ^ mul2(state[11]) ^ state[11] ^ state[15];
	result[8] = state[0] ^ state[4] ^ mul2(state[8]) ^ mul2(state[12]) ^ state[12];
	result[9] = state[1] ^ state[5] ^ mul2(state[9]) ^ mul2(state[13]) ^ state[13];
	result[10] = state[2] ^ state[6] ^ mul2(state[10]) ^ mul2(state[14]) ^ state[14];
	result[11] = state[3] ^ state[7] ^ mul2(state[11]) ^ mul2(state[15]) ^ state[15];
	result[12] = mul2(state[0]) ^ state[0] ^ state[4] ^ state[8] ^ mul2(state[12]);
	result[13] = mul2(state[1]) ^ state[1] ^ state[5] ^ state[9] ^ mul2(state[13]);
	result[14] = mul2(state[2]) ^ state[2] ^ state[6] ^ state[10] ^ mul2(state[14]);	
	result[15] = mul2(state[3]) ^ state[3] ^ state[7] ^ state[11] ^ mul2(state[15]);
	return result;
}

unsigned char* addKeySubShift(unsigned char currentState[16], unsigned char key[16]) {
	static unsigned char newState[16];
	newState[0] = S[currentState[0] ^ key[0]];
	newState[1] = S[currentState[1] ^ key[1]];
	newState[2] = S[currentState[2] ^ key[2]];
	newState[3] = S[currentState[3] ^ key[3]];
	newState[4] = S[currentState[5] ^ key[5]];
	newState[5] = S[currentState[6] ^ key[6]];
	newState[6] = S[currentState[7] ^ key[7]];
	newState[7] = S[currentState[4] ^ key[4]];
	newState[8] = S[currentState[10] ^ key[10]];
	newState[9] = S[currentState[11] ^ key[11]];
	newState[10] = S[currentState[8] ^ key[8]];
	newState[11] = S[currentState[9] ^ key[9]];
	newState[12] = S[currentState[15] ^ key[15]];
	newState[13] = S[currentState[12] ^ key[12]];
	newState[14] = S[currentState[13] ^ key[13]];
	newState[15] = S[currentState[14] ^ key[14]];
	return newState;
}

unsigned char* keySchedule(unsigned char currentKey[16], char roundConst) {
	static unsigned char newKey[16];
	newKey[0] = currentKey[0] ^ S[currentKey[7]] ^ roundConst;
	newKey[4] = currentKey[4] ^ S[currentKey[11]];
	newKey[8] = currentKey[8] ^ S[currentKey[15]];
	newKey[12] = currentKey[12] ^ S[currentKey[3]];
	newKey[1] = currentKey[1] ^ newKey[0];
	newKey[5] = currentKey[5] ^ newKey[4];
	newKey[9] = currentKey[9] ^ newKey[8];
	newKey[13] = currentKey[13] ^ newKey[12];
	newKey[2] = currentKey[2] ^ newKey[1];
	newKey[6] = currentKey[6] ^ newKey[5];
	newKey[10] = currentKey[10] ^ newKey[9];
	newKey[14] = currentKey[14] ^ newKey[13];
	newKey[3] = currentKey[3] ^ newKey[2];
	newKey[7] = currentKey[7] ^ newKey[6];
	newKey[11] = currentKey[11] ^ newKey[10];
	newKey[15] = currentKey[15] ^ newKey[14];
	return newKey;
}

int main(void) {
	unsigned char state[16] = {
			0xf3, 0x3c, 0xcd, 0x08, 
			0x44, 0xc6, 0x5d, 0xf2, 
			0x81, 0x27, 0xc3, 0x73, 
			0xec, 0xba, 0xfb, 0xe6
		}, 
		key[16] = {
			0x00, 0x00, 0x00, 0x00, 
			0x00, 0x00, 0x00, 0x00, 
			0x00, 0x00, 0x00, 0x00, 
			0x00, 0x00, 0x00, 0x00
		}, 
		*newState, *newKey, res[16];

	newState = addKeySubShift(state, key);
	newKey = keySchedule(key, 0x01);

	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x02);
	
	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x04);

	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x08);
	
	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x10);

	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x20);
	
	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x40);

	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x80);
	
	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x1B);

	newState = mixColumns(newState);
	newState = addKeySubShift(newState, newKey);
	newKey = keySchedule(newKey, 0x36);

	newState[0] ^= newKey[0];
	newState[1] ^= newKey[1];
	newState[2] ^= newKey[2];
	newState[3] ^= newKey[3];
	newState[4] ^= newKey[4];
	newState[5] ^= newKey[5];
	newState[6] ^= newKey[6];
	newState[7] ^= newKey[7];
	newState[8] ^= newKey[8];
	newState[9] ^= newKey[9];
	newState[10] ^= newKey[10];
	newState[11] ^= newKey[11];
	newState[12] ^= newKey[12];
	newState[13] ^= newKey[13];
	newState[14] ^= newKey[14];
	newState[15] ^= newKey[15];	

	// Only to make sure the encryption is correct and not considered part of optimized code
	for (int i = 0; i < 4; i++) {
		for (int j = 0; j < 4; j++) {
			res[4 * i + j] = newState[4 * j + i];
		}
	}
		
	return 0;
}

//Key			00000000000000000000000000000000
//Plaintext		f34481ec3cc627bacd5dc3fb08f273e6
//Ciphertext	0336763e966d92595a567cc9ce537f5e
